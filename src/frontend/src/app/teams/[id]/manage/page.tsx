...
export default function ManageTeamPage() {
	...
	const [activeTab, setActiveTab] = useState<
		"overview" | "members" | "applications" | "settings"
	>("overview");
		...
		const loadTeamData = async () => {
			try {
				...
				if (teamResponse.success && teamResponse.data) {
					...
					// Load applications
					const appsResponse = await getTeamApplications(
						teamData.id,
						user?.id?.toString()
					);
	...
	const handleInviteMember = async () => {
		...
		try {
			const response = await inviteToTeam(team.id.toString(), inviteEmail);
			if (response.success) {
				setInviteEmail("");
				setInviteMessage("");
		...
		recentActivity: [
			{
				...
				member: "NewMember",
				date: "2024-01-20",
				details: "Joined via application",
			},
			{
				type: "contest",
				member: "Team",
				date: "2024-01-18",
				details: "Participated in picoCTF 2024",
			},
			{
				type: "promotion",
				member: "SeniorMember",
				date: "2024-01-15",
				details: "Promoted to co-captain",
			},
		],
	};
					...
					>
						{[
							...
							},
							{
								...
								value: teamStats.pendingApplications,
								icon: Clock,
								color: "text-yellow-400",
							},
							{
								label: "Average Rating",
								value: teamStats.avgRating,
								icon: Star,
					...
					>
						<div className="flex space-x-1 bg-gray-800/30 border border-green-500/30 rounded-lg p-1">
							{[
								...
								{
									id: "applications",
									label: `Applications (${pendingApplications.length})`,
									icon: Mail,
								},
									...
									<div className="space-y-4">
										...
										<div className="flex justify-between">
											...
											<span className="font-mono text-gray-300">
												{team.statistics.totalPoints.toLocaleString()}
											</span>
										</div>
									</div>
								</div>
											...
											{team.captainId !== member.id.toString() && (
												<div className="flex items-center space-x-2">
													...
													<Button
														...
														size="sm"
														onClick={() =>
															setShowDeleteConfirm(member.id.toString())
														}
														disabled={actionLoading === member.id.toString()}
								...
								{pendingApplications.length === 0 ? (
									<div className="text-center py-12">
										...
										<p className="text-gray-500 font-mono">
											Applications will appear here when users apply to join
											your team
										</p>
									</div>
								) : (
									<div className="space-y-6">
										{pendingApplications.map((application) => (
											<div
												key={application.id}
												className="p-6 bg-gray-900/50 rounded-lg border border-gray-700"
											>
												<div className="flex items-start justify-between mb-4">
													<div>
														<h4 className="font-mono text-white font-bold text-lg">
												...
												<div className="mb-4">
													<h5 className="font-mono text-green-400 font-bold text-sm mb-2">
														MESSAGE:
													</h5>
													...
													<div className="flex flex-wrap gap-2">
														{application.skills.map((skill) => (
															<Badge
																key={skill}
																variant="outline"
																className="font-mono text-gray-300 border-gray-600 text-xs"
															>
																{skill}
															</Badge>
														))}
								...
								<div className="space-y-6">
									<div className="p-4 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
										<div className="flex items-center space-x-2 mb-2">
											...
											<h4 className="font-mono text-yellow-400 font-bold">
												Warning
											</h4>
										</div>
										<p className="font-mono text-yellow-300 text-sm">
											Team settings can be modified in the team edit page. Be
											careful when changing privacy settings or member limits.
										</p>
									</div>
									...
									<div className="border-t border-gray-700 pt-6">
										<h4 className="font-mono text-red-400 font-bold mb-4">
											DANGER ZONE
										</h4>
										<Button
											variant="outline"
					...
					<AnimatePresence>
						{showInviteModal && (
							...
							>
								...
								>
									<h3 className="font-mono text-green-400 font-bold text-lg mb-4">
										INVITE MEMBER
									</h3>
									...
									<div className="flex space-x-3 mt-6">
										<Button
											...
											disabled={
												!inviteEmail.trim() || actionLoading === "invite"
											}
