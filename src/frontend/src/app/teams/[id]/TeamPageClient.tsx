...
const CaptainCard = ({ captain }: { captain: TeamsDetailsUser }) => {
...
const MemberGrid = ({ members }: { members: TeamsDetailsUser[] }) => {
	const router = useRouter();
...
const ContestTimeline = ({ contests }: { contests: TeamsDetailsContest[] }) => {
	const router = useRouter();
	...
	const contestsByYear = contests.reduce(
		...
		{} as Record<number, TeamsDetailsContest[]>
	);
...
export default function TeamPageClient({ team }: { team: TeamDetails }) {
	return (
			...
			<div className="relative min-h-screen">
						...
						>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
								{[
									{
										label: "Contests",
										value: team.contest_history.length,
										icon: Target,
									},
									{
										label: "Avg Place",
										value: team.avg_place?.toFixed(1) ?? "--",
										icon: Trophy,
									},
									{
										label: "Members",
				...
				<section className="py-8">
					<div className="max-w-7xl mx-auto px-4">
										...
										{team.achievements.map((achievement, index) => (
											...
											>
												<div className="text-sm font-mono text-green-400">
													{achievement.name} (
													{new Date(achievement.unlocked_at).toDateString()})
												</div>
											</motion.div>
										))}
									</div>
						...
						{/* Contest History */}
						<ContestTimeline contests={team.contest_history} />
					</div>
				</section>
			</div>
		</div>
