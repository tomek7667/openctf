...
import { format, formatDistanceToNow, parseISO, formatDuration, intervalToDuration } from 'date-fns'
import { 
  ArrowLeft,
  Calendar, 
  Clock, 
...
const getMockPlaces = (contestId: number): Place[] => {
  if (contestId === 3 || contestId === 4) { // Only finished contests have places
    return [
      {
        id: 1,
...
const PlaceRow = ({ place, index }: { place: Place; index: number }) => {
  ...
  const getPlaceIcon = (position: number) => {
    switch (position) {
      ...
      case 2: return (
        <div className="w-5 h-5 rounded-full bg-gray-300 flex items-center justify-center text-black font-bold text-xs">2</div>
      )
      case 3: return (
        <div className="w-5 h-5 rounded-full bg-amber-600 flex items-center justify-center text-black font-bold text-xs">3</div>
      )
      default: return <span className="text-primary font-bold">#{position}</span>
    }
  }
  ...
  return (
    ...
    >
      ...
      <td className="p-4">
        <div className="font-mono font-bold text-foreground">{place.teamName}</div>
        {place.ctftimeTeamId && (
          <div className="text-xs text-muted-foreground">CTFtime ID: {place.ctftimeTeamId}</div>
        )}
      </td>
      ...
      <td className="p-4 font-mono text-right">
        <span className="text-primary font-bold">{place.openctfPoints?.toFixed(1) || 0}</span>
      </td>
    </motion.tr>
  )
}
      ...
      <MainLayout>
        <div className="min-h-screen flex items-center justify-center">
          <div className="terminal glass-terminal p-8 max-w-lg mx-auto text-center">
            <div className="text-primary mb-2">root@openctf:~# cat contest.txt</div>
            <p className="text-red-400 mb-4">
              {"// Contest not found"}<br/>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-4 mb-6">
              ...
              <h1 className="text-3xl md:text-4xl font-bold font-mono">
                ...
                <span className="hacker-gradient-text">{contest.name.replace(/-/g, '_').toUpperCase()}</span>
              </h1>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <div className={clsx(
                      ...
                      {contest.status === 'ongoing' && <Target className="h-4 w-4" />}
                      {contest.status === 'upcoming' && <Calendar className="h-4 w-4" />}
                      {contest.status === 'finished' && <Trophy className="h-4 w-4" />}
                ...
                {(contest.rules || contest.prizes) && (
                  <div className="space-y-4">
                    {contest.rules && (
                      <div>
                        <h3 className="font-bold font-mono text-primary mb-2">&gt; RULES</h3>
                    ...
                    {contest.prizes && (
                      <div>
                        <h3 className="font-bold font-mono text-primary mb-2">&gt; PRIZES</h3>
              ...
              <div className="space-y-6">
                <div className="bg-card/50 p-6 rounded-none hacker-border">
                  <h3 className="font-bold font-mono text-primary mb-4">&gt; CONTEST_INFO</h3>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="font-mono text-sm">{formatContestDate(contest.start)}</div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="font-mono text-sm">{contest.participantCount} teams</div>
                    </div>
                    ...
                    <table className="w-full">
                      <thead className="bg-muted/50">
                        <tr className="border-b border-border">
                          <th className="p-4 text-left font-mono text-sm font-bold">Rank</th>
                          <th className="p-4 text-left font-mono text-sm font-bold">Team Name</th>
                          <th className="p-4 text-right font-mono text-sm font-bold">Contest Points</th>
                          <th className="p-4 text-right font-mono text-sm font-bold">OpenCTF Points</th>
                        </tr>
                      </thead>
                      <tbody>
            ...
            <div className="max-w-7xl mx-auto text-center">
              <div className="terminal glass-terminal p-8 max-w-lg mx-auto">
                ...
                <p className="text-yellow-400">
                  {contest.status === 'upcoming' && "// Leaderboard will be available when contest starts"}
                  {contest.status === 'ongoing' && "// Live leaderboard coming soon"}
                  {contest.status === 'cancelled' && "// Contest was cancelled - no leaderboard available"}
                </p>
              </div>
            </div>
          </section>
