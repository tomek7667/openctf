...
function WriteupCard({ writeup }: { writeup: Writeup }) {
	const categoryInfo =
		categories.find((c) => c.id === writeup.category) || categories[0];
	const difficultyInfo =
		difficulties.find((d) => d.id === writeup.difficulty) || difficulties[0];
			...
			<div className="relative bg-card/50 backdrop-blur-sm border border-green-500/30 rounded-lg p-6 h-full hover:border-green-400/50 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-green-500/20 flex flex-col">
				...
				<div className="flex items-start justify-between mb-4">
					<div className="flex items-center space-x-3">
						<div
							className={`p-2 rounded-lg bg-gradient-to-r ${categoryInfo?.color || "from-gray-500 to-gray-600"} bg-opacity-20 border border-current/30`}
						>
						...
						<div>
							...
							<div className="flex items-center space-x-2 mt-1">
								...
								<span
									className={`text-xs font-mono ${difficultyInfo?.color || "text-gray-400"}`}
								>
				...
				<div className="flex flex-wrap gap-1 mb-4">
					{writeup.tags.slice(0, 4).map((tag) => (
						<Badge
							key={tag}
							variant="outline"
							className="text-xs font-mono text-gray-400 border-gray-600"
						>
							#{tag}
						</Badge>
					))}
					...
					<div className="flex items-center space-x-4 mb-4 text-sm">
						...
						{writeup.contestName && (
							<div className="flex items-center space-x-1 text-gray-400">
								...
								<Link
									href={`/contests/${writeup.contestId || "#"}`}
									className="font-mono text-lime-400 underline hover:text-lime-300 transition-colors"
								>
									{writeup.contestName}
								</Link>
							</div>
							...
							{writeups.length === 0 && (
								<div className="text-center py-12">
									...
									<p className="text-gray-500 font-mono">
										Try adjusting your search criteria or create a new writeup
									</p>
								</div>
							)}
