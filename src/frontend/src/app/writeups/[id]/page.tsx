						...
						<div className="bg-card/30 border border-border rounded-lg p-8">
							<div className="flex items-start justify-between mb-6">
								<div className="flex items-start space-x-4 flex-1">
									<div
										className={`p-3 rounded-lg bg-gradient-to-r ${categoryInfo?.color || "from-gray-500 to-gray-600"} bg-opacity-20 border border-current/30`}
									>
								...
								<div className="flex items-center space-x-4">
									{isAuthenticated &&
										user?.id?.toString() === writeup.authorId && (
											<Button
												onClick={() =>
													router.push(`/writeups/${writeup.id}/edit`)
												}
												variant="outline"
							...
							<div className="flex flex-wrap gap-2 mt-4">
								{writeup.tags.map((tag) => (
									<Badge
										key={tag}
										variant="outline"
										className="font-mono text-gray-400 border-gray-600"
									>
										#{tag}
									</Badge>
								))}
