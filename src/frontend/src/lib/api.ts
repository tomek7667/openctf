...
type TeamsResponse = PaginatedResponse<Team>;
type ContestsResponse = PaginatedResponse<Contest>;
...
class ApiClient {
	...
	// Teams endpoints
	async getTeams(params?: undefined | ListTeamsDto): Promise<TeamsResponse> {
		const response = await this.api.get<TeamsResponse>("/teams/list", {
			params,
		});
	...
	async createTeam(teamData: CreateTeamDto): Promise<{ team: Team }> {
		const response = await this.api.post<{ team: Team }>(
			"/teams/create",
			teamData
		);
		return response.data;
	...
	async getContests(
		...
		const response = await this.api.get<ContestsResponse>("/contests/list", {
			params,
		});
		return response.data;
	}
	...
	async getContest(contestId: number): Promise<{ contest: Contest }> {
		const response = await this.api.get<{ contest: Contest }>(
			`/contests/${contestId}`
		);
		return response.data;
	}
	...
	async createContest(
		...
		const response = await this.api.post<{ contest: Contest }>(
			"/contests/create",
			contestData
		);
		return response.data;
