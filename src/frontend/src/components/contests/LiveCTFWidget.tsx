...
export function LiveCTFWidget({ contests }: LiveCTFWidgetProps) {
  ...
  const groupedUpcoming = upcomingContests.reduce((acc, contest) => {
  ...
  const upcomingToShow = Object.entries(groupedUpcoming)
    .slice(0, 3)
    .flatMap(([, contests]) => contests.length >= 5 ? contests : contests.slice(0, 3));
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="glass-terminal p-6 rounded-none hacker-border">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-red-400 font-mono font-bold">LIVE CONTESTS</span>
            </div>
                ...
                <Link key={contest.id} href={`/contests/${contest.id}`}>
                  <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-none cursor-pointer hover:bg-red-500/20 transition-colors">
                    ...
                    <div className="text-xs text-muted-foreground font-mono mt-1">
                      Ends: {new Date(contest.endTime).toLocaleDateString()} {new Date(contest.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} | W: {contest.weight || 0}
                    </div>
                  </div>
                </Link>
              ))}
        ...
        <div className="glass-terminal p-6 rounded-none hacker-border">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-blue-400 font-mono font-bold">UPCOMING CONTESTS</span>
            <span className="text-muted-foreground font-mono text-sm">
                ...
                return (
                  <Link key={contest.id} href={`/contests/${contest.id}`}>
                    <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-none cursor-pointer hover:bg-blue-500/20 transition-colors">
                      ...
                      <div className="text-xs text-muted-foreground font-mono mt-1">
                        Starts: {new Date(contest.startTime).toLocaleDateString()} {new Date(contest.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} | {contest.duration}h | W: {contest.weight || 0}
                      </div>
                    </div>
                  </Link>
                );
