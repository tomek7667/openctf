...
export const TestToastButton = () => {
	...
	const showTestToasts = () => {
		...
		setTimeout(() => {
			toast.error(
				"Connection Failed",
				"Failed to connect to leaderboard service. Network timeout."
			);
		}, 1500);
		...
		setTimeout(() => {
			toast.info(
				"System Update",
				"Leaderboard rankings are being recalculated."
			);
		}, 3500);
	};

	return (
		<Button onClick={showTestToasts} className="btn-terminal text-xs px-4 py-2">
			[TEST_NOTIFICATIONS]
		</Button>
	);
};
