...
import { MAIN_NAVIGATION, USER_NAVIGATION, ADMIN_NAVIGATION } from '@/constants/navigation'
  ...
  const filteredNavigation = useMemo(() => {
    const filterByAuth = (items: NavigationItem[]) => 
      items.filter(item => {
        ...
        if (item.adminOnly && user?.role !== 'admin') return false // Assuming role field exists
        return true
      })
