...
export interface ApiClientConfig {
	...
	enableCache?: undefined | boolean;
	enableRateLimit?: undefined | boolean;
	defaultHeaders?: undefined | Record<string, string>;
}

export interface RequestConfig extends AxiosRequestConfig {
	skipAuth?: undefined | boolean;
	skipRetry?: undefined | boolean;
	skipErrorToast?: undefined | boolean;
	cacheKey?: undefined | string;
	cacheTTL?: undefined | number; // in milliseconds
}
...
export class ApiClientError extends Error {
	constructor(
		...
		public details?: undefined | Record<string, unknown>
	) {
		super(message);
...
export class ApiClient {
	...
	private setupInterceptors(): void {
		...
		this.client.interceptors.request.use(
			(config) => {
				...
				// Add auth token
				if (this.authToken && !customConfig.skipAuth) {
	...
	public async get<T>(url: string, config: RequestConfig = {}): Promise<T> {
		const cacheKey =
			config.cacheKey || this.generateCacheKey("GET", url, config.params);
	...
	private async request<T>(config: RequestConfig): Promise<T> {
		...
		for (
			let attempt = 0;
			attempt <= (skipRetry ? 0 : this.retryConfig.retries);
			attempt++
		) {
			try {
	...
	public cancelAllRequests(): void {
		for (const [_, cancelToken] of this.cancelTokens.entries()) {
