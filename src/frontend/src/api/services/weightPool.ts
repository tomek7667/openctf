...
import { Contest, AggregatedContestsDifficulties } from "@/types/api";
...
export interface ContestWeightHistory {
	...
	avgQuality?: undefined | number; // from ContestRating aggregation
...
export class WeightPoolApiService {
		...
		return [
			// January 2025 contests
			{
				contest: {
					...
					prizes:
						"$10,000 for first place, $5,000 for second, $2,500 for third",
	...
	async getContestsByMonth(
		...
		return allContests.filter((item) => {
			...
			const contestMonth = contestDate.toLocaleString("en-US", {
				month: "long",
			});
	 ...
	 */
	async getAggregatedContestsDifficulties(): Promise<
