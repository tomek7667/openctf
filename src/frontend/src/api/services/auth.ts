...
export class AuthApiService {
  ...
  async requestPasswordReset(email: string): Promise<{ success: boolean; message: string }> {
    return apiClient.post<{ success: boolean; message: string }>('/auth/password/reset', {
      email,
    }, {
      skipAuth: true,
    });
   ...
   * Update user profile
   */
  async updateProfile(userData: Partial<Pick<User, 'username' | 'email' | 'description'>>): Promise<User> {
  ...
  async enableTwoFactor(): Promise<{ qrCode: string; backupCodes: string[] }> {
    return apiClient.post<{ qrCode: string; backupCodes: string[] }>('/auth/2fa/enable');
  }
