...
export class ContestsApiService {
	 ...
	 */
	async getContests(
		params?: undefined | ListContestsDto
		...
		const mockContests: Contest[] = [
			// Ongoing Contest
			{
				...
				description:
					"International cybersecurity competition featuring advanced exploitation challenges",
			...
			// Upcoming Contest
			{
				...
				description:
					"Annual university-level competition with beginner-friendly challenges and educational content",
			...
			// Finished Contest
			{
				...
				description:
					"Annual BSides San Francisco Capture The Flag competition with industry-standard challenges",
			...
			// Another Finished Contest
			{
				...
				name: "picoctf-2024",
				description:
					"Educational CTF for beginners and advanced players alike, featuring progressive difficulty levels",
			...
			// Another Upcoming Contest
			{
				...
				description:
					"Week-long global cybersecurity challenge with 24/7 support and live streams",
	...
	async updateContest(
		contestId: number,
		contestData: Partial<CreateContestDto>
	): Promise<Contest> {
	...
	// 	contestId: number,
	// 	limit: number = 100
	// ): Promise<ContestPlacement[]> {
	// 	return apiClient.get<ContestPlacement[]>(
	...
	async isRegistered(
		...
		return apiClient.get<{ registered: boolean }>(
	 ...
	 */
	async getContestParticipants(contestId: number): Promise<
		Array<{
			team: {
				id: number;
				name: string;
	 ...
	 */
	async getContestRatings(contestId: number): Promise<
		Array<{
			id: number;
			difficulty: number;
			quality: number;
	 ...
	 */
	async getContestWriteups(contestId: number): Promise<
		Array<{
			id: number;
			title: string;
			author: {
	 ...
	 */
	async getContestTimeline(contestId: number): Promise<
		Array<{
			id: number;
	 ...
	 */
	// async getContestChallenges(contestId: number): Promise<
	// 	Array<{
	// 		id: number;
	// 		name: string;
	// 		category: CTFCategory;
	...
	async syncWithCtfTime(
		...
		return apiClient.post<{ success: boolean; changes?: undefined | string[] }>(
			`/contests/${contestId}/sync-ctftime`
		);
	}
	 ...
	 */
	async toggleContestFollow(
		contestId: number,
		follow: boolean
	): Promise<{ success: boolean }> {
		return apiClient.post<{ success: boolean }>(
			`/contests/${contestId}/follow`,
			{
				follow,
			}
		);
