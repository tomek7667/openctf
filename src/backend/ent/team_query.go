...
func (_q *TeamQuery) QueryVerifiedBy() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withVerifiedBy: _q.withVerifiedBy.Clone(),
...
func (_q *TeamQuery) WithVerifiedBy(opts ...func(*UserQuery)) *TeamQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withVerifiedBy = query
	return _q
}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	var (
		...
		loadedTypes = [3]bool{
			_q.withCaptain != nil,
			_q.withVerifiedBy != nil,
			_q.withMembers != nil,
		}
	)
	if _q.withCaptain != nil || _q.withVerifiedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withVerifiedBy; query != nil {
		if err := _q.loadVerifiedBy(ctx, query, nodes, nil,
...
func (_q *TeamQuery) loadCaptain(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	...
	for i := range nodes {
		if nodes[i].team_captain == nil {
			continue
		}
		fk := *nodes[i].team_captain
...
func (_q *TeamQuery) loadVerifiedBy(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Team)
	for i := range nodes {
