...
func (_q *UserProfileQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *UserProfileQuery) FirstIDX(ctx context.Context) int {
...
func (_q *UserProfileQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *UserProfileQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *UserProfileQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *UserProfileQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
//		GroupBy(userprofile.FieldLocation).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.UserProfile.Query().
//		Select(userprofile.FieldLocation).
//		Scan(ctx, &v)
...
func (_q *UserProfileQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*UserProfile, init func(*UserProfile), assign func(*UserProfile, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_profile_user == nil {
			continue
		}
		fk := *nodes[i].user_profile_user
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_profile_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
