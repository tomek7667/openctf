...
fixed weight points pool from which weights are [_somehow_](#actual-difficulty-rating---formula) given to particular events, especially to those known for their quality and level.
...
The above types are TODO: to be tagged somehow in future for the organizers to inform newcomers what can they expect from a ctf based on the previous ones.

### actual difficulty rating - formula

at the end of each month, each contest will receive weight points from the weight pool which is 100 each month.

the contest is taken into weight pool consideration only if the following conditions apply:

- the organizers of the contest have organized at least one different ctf with at least 50 teams at least 6 months before
- the contest have at least 50 teams participating

top 15 captains or top 5% _(whichever is smaller)_ in a contest vote how **hard** _(exluding guessy==hard; guessy=0)_ was the contest from 0 being the easiest to 100 being the hardest they've played in last 2 years.
